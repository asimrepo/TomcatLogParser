...
class every():
    ...
    # will replace it with * otherwise it will show the original value passed as argument to the function
        ...
        for i in range(len(tmp7)):
            ...
            previousvalue=int(tmp6[i])
            result.append(previousvalue)
        totalcount=len(tmp7)
        print("average time took " +str(round(sum(result)/totalcount))+'ms' +' and number of invocations are ' +str(totalcount) +' for URL ' +str(reg))

for i in range(len(tmp4)):
    every.repetition(tmp4[i])
